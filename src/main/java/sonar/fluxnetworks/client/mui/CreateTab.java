...
public class CreateTab extends Fragment {
    ...
    private RadioGroup mColorGroup;  // Mudança: RelativeRadioGroup -> RadioGroup
    ...
    public void onCreate(@Nullable DataSet savedInstanceState) {
        ...
        getParentFragmentManager().setFragmentResultListener("create_network", this, (requestKey, result) -> {
            ...
            if (t != null) {
                MUIIntegration.showToastError(requireContext(), t);
            }
        });
    }
    ...
    public View onCreateView(@Nonnull LayoutInflater inflater,
...

        {
            ...
            if (player != null) {
                text = player.getGameProfile().name() + "'s Network";  // Mudança: getName() -> name()
            }
            mName.setText(text);
    ...
    @Nonnull
    private RadioGroup inflateColorGroup() {  // Mudança: RelativeRadioGroup -> RadioGroup
        var group = new RadioGroup(requireContext());  // Mudança: RadioGroup em vez de RelativeRadioGroup

        // RadioGroup usa orientação por padrão, então configuramos para múltiplas linhas
        group.setOrientation(RadioGroup.VERTICAL);

        // Criar um LinearLayout interno para organizar os botões em grade
        var row1 = new LinearLayout(requireContext());
        row1.setOrientation(LinearLayout.HORIZONTAL);
        var row2 = new LinearLayout(requireContext());
        ...
        for (int i = 0; i < EnumNetworkColor.VALUES.length; i++) {
            ...
            // Distribuir entre as duas linhas
            if (i < 7) {
                row1.addView(v);
            } else {
                row2.addView(v);
        ...
        group.setOnCheckedChangeListener((radioGroup, checkedId) -> {
            if (checkedId > 0) {
                mSelectedColor = EnumNetworkColor.VALUES[checkedId - 1].getRGB();
