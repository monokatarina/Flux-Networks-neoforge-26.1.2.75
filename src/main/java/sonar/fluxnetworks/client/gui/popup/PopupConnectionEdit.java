    ...
    public void drawForegroundLayer(@Nonnull GuiGraphicsExtractor gr, int mouseX, int mouseY, float deltaTicks) {
        ...
        /*if (!mHost.mBatchMode) {
            drawCenteredString(poseStack, font, FluxTranslate.SINGLE_EDIT.get(),
                    leftPos + 88, topPos + 14, 0xFFFFFFFF);
            drawCenteredString(poseStack, font, FluxUtils.getDisplayPos(mHost.mSingleConnection.getGlobalPos()),
                    leftPos + 88, topPos + 121, 0xFFFFFFFF);
            drawCenteredString(poseStack, font, FluxUtils.getDisplayDim(mHost.mSingleConnection.getGlobalPos()),
                    leftPos + 88, topPos + 130, 0xFFFFFFFF);
        } else {*/
        // Usar centeredText em vez de drawCenteredString
        gr.centeredText(font, FluxTranslate.BATCH_EDIT.get(),
                leftPos + 88, topPos + 14, 0xFFFFFFFF);
        gr.centeredText(font,
                FluxTranslate.EDITING_CONNECTIONS.format(mHost.mSelected.size()),
                leftPos + 88, topPos + 122, 0xFFFFFFFF);
        //}
        // Usar drawString (agora text()) em vez de drawString
        gr.text(font, FluxTranslate.SURGE_MODE.get(),
                leftPos + 20, topPos + 82, opaqueColor(mHost.getNetwork().getNetworkColor()));
        gr.text(font, FluxTranslate.DISABLE_LIMIT.get(),
                leftPos + 20, topPos + 94, opaqueColor(mHost.getNetwork().getNetworkColor()));
        if (mChunkLoading != null) {
            gr.text(font, FluxTranslate.CHUNK_LOADING.get(),
                    leftPos + 20, topPos + 106, opaqueColor(mHost.getNetwork().getNetworkColor()));
        }
    }
