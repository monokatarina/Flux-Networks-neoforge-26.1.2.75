...
public class PageLabelButton extends GuiButtonCore {
    ...
    @Override
    public void drawButton(GuiGraphicsExtractor gr, int mouseX, int mouseY, float deltaTicks) {
        int pages = mPages;
        if (pages <= 0) {
            return;
        }
        ...
        if (mHoveredPage != -1) {
            ...
            gr.centeredText(screen.getMinecraft().font, (mHoveredPage + 1) + " / " + pages,
                    x + width / 2, y + 6, mColor);
        } else if (mShowTick > 0) {
            int alpha = (int) Math.min(255, mShowTick * 24);
            if (alpha > 3) {
                // Para texto com alpha, usar text() em vez de centeredText e calcular posição manualmente
                String text = (mPage + 1) + " / " + pages;
                int textWidth = screen.getMinecraft().font.width(text);
                int colorWithAlpha = (alpha << 24) | (mColor & 0x00FFFFFF);
                gr.text(screen.getMinecraft().font, text,
                        x + (width - textWidth) / 2, y + 6, colorWithAlpha, true);
            }
