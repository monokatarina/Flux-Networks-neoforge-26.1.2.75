    ...
    protected void drawForegroundLayer(GuiGraphicsExtractor gr, int mouseX, int mouseY, float deltaTicks) {
        ...
        if (mCustomName.getValue().isEmpty()) {
            ...
            gr.text(font,
                    ...
                    mCustomName.getX() + 4, y, opaqueColor(FluxConstants.INVALID_NETWORK_COLOR));
        }

        // Usar text() em vez de drawString
        gr.text(font, FluxTranslate.SURGE_MODE.get(), 20 + leftPos, 120 + topPos, color);
        gr.text(font, FluxTranslate.DISABLE_LIMIT.get(), 20 + leftPos, 132 + topPos, color);

        if (mChunkLoading != null) {
            gr.text(font, FluxTranslate.CHUNK_LOADING.get(), 20 + leftPos, 144 + topPos, color);
        }
    }
