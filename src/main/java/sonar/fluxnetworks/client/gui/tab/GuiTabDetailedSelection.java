...
public class GuiTabDetailedSelection extends GuiTabSelection {
    ...
    protected void renderBarAndName(GuiGraphicsExtractor gr, FluxNetwork element, int x, int y, boolean selected) {
        ...
        // CORREÇÃO: pushMatrix() em vez de pushPose()
        gr.pose().pushMatrix();
        // CORREÇÃO: scale com 2 argumentos (removeu o terceiro parâmetro)
        ...
        // Usar text() com escala
        gr.text(font, text,
                (int) ((x + 4) / 0.75f), (int) ((y + 11) / 0.75f), selected ? 0xFFFFFFFF : 0xFF808080, true);
        // CORREÇÃO: popMatrix() em vez de popPose()
        gr.pose().popMatrix();
    }
