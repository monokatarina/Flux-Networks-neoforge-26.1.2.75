...
public class GuiTabWireless extends GuiTabCore {
    ...
    protected void drawForegroundLayer(GuiGraphicsExtractor gr, int mouseX, int mouseY, float deltaTicks) {
        ...
        if (getNetwork().isValid()) {
            int color = opaqueColor(getNetwork().getNetworkColor());
            gr.centeredText(font, FluxTranslate.TAB_WIRELESS.get(), leftPos + 88, topPos + 10, 0xFFB4B4B4);
            gr.text(font, FluxTranslate.ENABLE_WIRELESS.get(), leftPos + 20, topPos + 148, color);
            ...
            if (WirelessType.ENABLE_WIRELESS.isActivated(ClientCache.sWirelessMode) &&
                ...
                gr.centeredText(font,
                        ...
                        leftPos + 88, topPos + 158, 0xFFB4B4B4);
            }
        } else {
    ...
    public void init() {
        ...
        if (getNetwork().isValid()) {
            ...
            mEnable = new SwitchButton(this, leftPos + 140, topPos + 148,
                    WirelessType.ENABLE_WIRELESS.isActivated(mWirelessMode), opaqueColor(getNetwork().getNetworkColor()));
    ...
    protected void containerTick() {
        super.containerTick();
        if (mEnable != null) {
            mEnable.setColor(opaqueColor(getNetwork().getNetworkColor()));
        }
    }
}
