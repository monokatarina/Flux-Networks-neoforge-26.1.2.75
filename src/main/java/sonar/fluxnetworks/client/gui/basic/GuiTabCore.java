...
public abstract class GuiTabCore extends GuiFluxCore {
    ...
    protected void renderNavigationPrompt(GuiGraphicsExtractor gr, @Nonnull FluxTranslate error,
                                          @Nonnull EnumNavigationTab tab) {
        gr.text(font, error.get(), width / 2 - font.width(error.get()) / 2, topPos + 16, 0xFF808080);
        gr.text(font, FluxTranslate.CLICK_ABOVE.format(ChatFormatting.AQUA + tab.getTranslatedName() + ChatFormatting.RESET),
                width / 2 - font.width(FluxTranslate.CLICK_ABOVE.format(ChatFormatting.AQUA + tab.getTranslatedName() + ChatFormatting.RESET)) / 2,
                topPos + 28, 0xFF808080);
    }
