...
public class GuiFluxAdminHome extends GuiTabCore {
    ...
    protected void drawForegroundLayer(GuiGraphicsExtractor gr, int mouseX, int mouseY, float deltaTicks) {
        ...
        gr.text(font, AccessLevel.SUPER_ADMIN.getFormattedName(), leftPos + 20, topPos + 30, color);
        gr.text(font, FluxTranslate.DETAILED_VIEW.get(), leftPos + 20, topPos + 42, color);
    }
    ...
    public void init() {
        ...
        int color = opaqueColor(getNetwork().getNetworkColor());
        mSuperAdmin = new SwitchButton(this, leftPos + 140, topPos + 30, superAdmin, color);
        mButtons.add(mSuperAdmin);

        mDetailedNetworkView = new SwitchButton(this, leftPos + 140, topPos + 42,
                ClientCache.sDetailedNetworkView, color);
