...
public class ClientMessages {
    ...
    public static void itemNetwork(int token, FluxNetwork network, String password) {
        var buf = Channel.buffer(Messages.C2S_ITEM_NETWORK);
        buf.writeByte(token);
        buf.writeVarInt(network.getNetworkID());
        buf.writeUtf(password, 256);
        sChannel.sendToServer(buf);
