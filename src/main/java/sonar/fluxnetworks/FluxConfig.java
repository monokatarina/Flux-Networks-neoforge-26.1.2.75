...
public class FluxConfig {
    ...
    static {
        ...
        // CORRIGIDO: FMLEnvironment.getDist() em vez de FMLLoader.getDist()
    ...
    static void init(ModContainer modContainer) {
        // CORRIGIDO: FMLEnvironment.getDist() em vez de FMLLoader.getDist()
        if (FMLEnvironment.getDist() == Dist.CLIENT && CLIENT_SPEC != null) {
            modContainer.registerConfig(ModConfig.Type.CLIENT, CLIENT_SPEC);
        }
        modContainer.registerConfig(ModConfig.Type.COMMON, COMMON_SPEC);
        modContainer.registerConfig(ModConfig.Type.SERVER, SERVER_SPEC);
        modContainer.getEventBus().addListener(FluxConfig::reload);
    }

    static void reload(@Nonnull ModConfigEvent event) {
        // Recarrega todas as configurações independentemente do tipo
        switch (event.getConfig().getType()) {
            case CLIENT -> {
                if (CLIENT_CONFIG != null && CLIENT_SPEC != null && CLIENT_SPEC.isLoaded()) {
                    CLIENT_CONFIG.load();
    ...
    public static boolean enableButtonSound = true, enableGuiDebug = false;
    public static boolean enableOneProbeBasicInfo = true, enableOneProbeAdvancedInfo = true, enableOneProbeSneaking = true;
    public static boolean enableFluxRecipe = true, enableChunkLoading = true, enableSuperAdmin = true;
    public static long defaultLimit = 800000, basicCapacity = 2000000, basicTransfer = 20000,
            herculeanCapacity = 16000000, herculeanTransfer = 120000,
            gargantuanCapacity = 128000000, gargantuanTransfer = 720000;
    public static int maximumPerPlayer = 5, superAdminRequiredPermission = 1;
