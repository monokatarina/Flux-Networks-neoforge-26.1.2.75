...
public abstract class TransferHandler {
    ...
    public void readCustomTag(@Nonnull CompoundTag tag, byte type) {
        ...
        if (tag.contains(FluxConstants.BUFFER)) {
            ...
            mBuffer = tag.getLongOr(FluxConstants.ENERGY, 0L);
        }
        switch (type) {
            case FluxConstants.NBT_SAVE_ALL, FluxConstants.NBT_TILE_DROP -> {
                mPriority = tag.getIntOr(FluxConstants.PRIORITY, mPriority);
                mSurgeMode = tag.getBooleanOr(FluxConstants.SURGE_MODE, mSurgeMode);
                mLimit = tag.getLongOr(FluxConstants.LIMIT, mLimit);
                mDisableLimit = tag.getBooleanOr(FluxConstants.DISABLE_LIMIT, mDisableLimit);
            }
            case FluxConstants.NBT_TILE_UPDATE -> {
                mChange = tag.getLongOr(FluxConstants.CHANGE, mChange);
                mPriority = tag.getIntOr(FluxConstants.PRIORITY, mPriority);
                mSurgeMode = tag.getBooleanOr(FluxConstants.SURGE_MODE, mSurgeMode);
                mLimit = tag.getLongOr(FluxConstants.LIMIT, mLimit);
                mDisableLimit = tag.getBooleanOr(FluxConstants.DISABLE_LIMIT, mDisableLimit);
    ...
    public boolean changeSettings(@Nonnull CompoundTag tag) {
        ...
        if (tag.contains(FluxConstants.SURGE_MODE)) {
            sort = setSurgeMode(tag.getBooleanOr(FluxConstants.SURGE_MODE, mSurgeMode));
        }
        if (tag.contains(FluxConstants.PRIORITY)) {
            sort |= setPriority(tag.getIntOr(FluxConstants.PRIORITY, mPriority));
        }
        if (tag.contains(FluxConstants.DISABLE_LIMIT)) {
            setDisableLimit(tag.getBooleanOr(FluxConstants.DISABLE_LIMIT, mDisableLimit));
        }
        if (tag.contains(FluxConstants.LIMIT)) {
            setLimit(tag.getLongOr(FluxConstants.LIMIT, mLimit));
        }
        return sort;
    }
