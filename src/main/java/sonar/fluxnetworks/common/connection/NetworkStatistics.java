...
public class NetworkStatistics {
    ...
    public void writeNBT(CompoundTag tag) {
        ...
        tag.putLongArray("a", energyChange.toLongArray());
    }

    public void readNBT(CompoundTag tag) {
        fluxPlugCount = tag.getIntOr("1", 0);
        fluxPointCount = tag.getIntOr("2", 0);
        fluxControllerCount = tag.getIntOr("3", 0);
        fluxStorageCount = tag.getIntOr("4", 0);
        energyInput = tag.getLongOr("5", 0L);
        energyOutput = tag.getLongOr("6", 0L);
        totalBuffer = tag.getLongOr("7", 0L);
        totalEnergy = tag.getLongOr("8", 0L);
        averageTickMicro = tag.getIntOr("9", 0);
        long[] a = tag.getLongArray("a").orElse(new long[0]);
        for (int i = 0; i < Math.min(a.length, CHANGE_COUNT); i++) {
