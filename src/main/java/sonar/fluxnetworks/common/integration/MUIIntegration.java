...
public class MUIIntegration {
    ...
    public static void showToastError(@Nonnull FluxTranslate translate) {
        showToastError(null, translate);
    }

    @SuppressWarnings("removal")
    public static void showToastError(@Nullable Context context, @Nonnull FluxTranslate translate) {
        ...
        if (Core.isOnUiThread()) {
            makeToast(context, text).show();
        } else {
            MuiForgeApi.postToUiThread(() -> makeToast(context, text).show());
        }
    }

    @SuppressWarnings("removal")
    private static Toast makeToast(@Nullable Context context, CharSequence text) {
        return context != null
                ? Toast.makeText(context, text, Toast.LENGTH_SHORT)
                : Toast.makeText(text, Toast.LENGTH_SHORT);
    }
